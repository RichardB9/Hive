(function (global, factory) {
	...
	function getX (hex, layout, hexWidth, hexRadius) {
		...
		switch (layout) {
			case "odd-r":
				xOffset = ( (hex.rc & 1) === 1) ? hexWidth : (hexWidth / 2);
				x = (hex.qc * hexWidth) + xOffset;
				break;

			case "even-r":
				xOffset = ((hex.rc & 1) === 0) ? hexWidth : (hexWidth / 2);
				x = (hex.qc * hexWidth) + xOffset;
				break;
	...
	function getY (hex, layout, hexWidth, hexRadius) {
		...
		switch (layout) {
			...
			case "odd-q":
				yOffset = ( (hex.qc & 1) === 1) ? hexWidth : (hexWidth / 2);
				y = (hex.rc * hexWidth) + yOffset;
				break;

			case "even-q":
				yOffset = ( (hex.qc % 1) === 0) ? hexWidth : (hexWidth / 2);
				y = (hex.rc * hexWidth) + yOffset;
				break;
		}
